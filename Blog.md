...
Giving agents access to a sandboxed environment with a shell and a filesystem has been the latest hype when it comes to agentic harnesses. Recent examples:
...
When you try to fit these into a sandboxed filesystem, tricky questions come up fast:
...
[A reply](https://x.com/SeeLos/status/2009806164935000211?s=20) to a recent X post from Vercel about building filesystem-based agents puts it well:
...
This post will show a pattern to close that gap: making anything a filesystem with the help of FUSE.

## An example: Email agent
To make this tangible, let's build a fictional agent.

Imagine we're building an AI-powered email platform, think Gmail but with nice AI on top. We have our own database storing emails, folders, and contacts. We have a UI that users interact with.

Now we want to add a powerful agent. We could build a simple agent loop with tools like `list-emails`, `move-email`, `read-email`, but as we established there's a more ergonomic approach: put our emails in a filesystem.

After some thinking, we come up with the following filesystem layout for the agent's sandbox:
...
[FUSE](https://www.kernel.org/doc/html/v6.3/filesystems/fuse.html) stands for *Filesystem in Userspace*. It's a framework that lets you implement filesystems outside the kernel, while the kernel makes it look like a 'real' filesystem to the rest of Linux. You just write a userland process that implements the actual filesystem logic.
...
There are a few more operations like `open`, `mkdir`, and `rename` to allow moving email files around and reassigning folder IDs in the database.
...
![[Bildschirmaufnahme 2026-01-11 um 19.29.58.mov]]
...
## Outlook
You can find the full implementation on GitHub: https://github.com/Jakob-em/agent-fuse to play aroun with yourself. I also might write another post about evaling this approach vs a traditional agent loop.

In genreal there is a lot more potential for virtual filesystems for context engineering. You could include old conversations as files on the filesystem, or make old tool call results appear as files after compacting context. The possibilities are endless, and as long as coding agents kremain relevant, this way of designing context will only get more ergonomic for models.
...
No worrying about FUSE, the sandbox, where things are executed, etc. This will be a huge differentiator and make virtual filesystems accessible to everyone.
